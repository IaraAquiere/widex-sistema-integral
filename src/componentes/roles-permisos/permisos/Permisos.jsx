...
const Permisos = () => {
  ...
  const GetData = () => {
    ...
    fetch("HTTP://localhost:5000/Permisos/ListarPorRol/" + routeParams.pId_rol, requestOptions)
  ...
  const clickhandler = (permiso) => {
    const pos = permisos.findIndex((item) => item.id_permiso === permiso.id_permiso);
    ...
    <>
      {routeParams.pAccion <= 0   ? 
       <div className="container">
        <p>Nombre Rol:</p> <input type="text" value = {NombreRol} onChange={ cambioNombreRol }/>
       </div>
      : <div className="container"><h1>{NombreRol}</h1></div>
      }
      ...
      <div className="d-flex justify-content-end">
        <div className="guardar-permisos">
          <button className="btn btn-success" onClick={() => GuardarData()}>Guardar</button>
        </div>
      </div>
    </>
  );
